...
Community.Behavior.PraiseEvent = function PraiseEvent(event) {
    ...
    var img_$praise_content = event.data.Index ? $("#liCommunityPraise" + event.data.Index).find(".opt-text") : event.data.Index;
...
Community.Behavior.TreadEvent = function TreadEvent(event) {
    ...
    var img_$thread_content = event.data.Index ? $("#liCommunityTread" + event.data.Index).find(".opt-text") : event.data.Index;
...
Community.Behavior.CollectEvent = function CollectEvent(event) {
    ...
    var img_$collect_content = event.data.Index ? $("#liCommunityCollect" + event.data.Index).find(".opt-text") : event.data.Index;
...
Community.Behavior.ReportEvent = function ReportEvent(event) {
    ...
    var img_$report_content = event.data.Index ? $("#liCommunityReport" + event.data.Index).find(".opt-text") : event.data.Index;
    var publish_id = event.data.PublishID;
...
Community.Behavior.CommentViewEvent = function CommentViewEvent(event) {
    ...
    if ($open.is(":hidden") == false) {
        ...
        if ($(this).data("initial") == undefined) {
            ...
            $("#commentSend" + event.data.PublishIndex).off("click").on("click", { PublishID: publish_id, List: $comment_list, Editor: $comment_div.find(".weibo-editor-comment"), ToUserID: to_user_id, ToUserName: to_user_name, PublishIndex: event.data.PublishIndex }, Community.Behavior.CommentEvent);
            $("#commentCancel" + event.data.PublishIndex).off("click").on("click", { Editor: $comment_div.find(".weibo-editor-comment"), ToUserID: to_user_id, ToUserName: to_user_name, PublishIndex: event.data.PublishIndex }, Community.Behavior.CancelCommentEvent).hide();
...
Community.Behavior.GetComment = function get_comment($comment_list, publish_id, to_user_id, page, publish_index, callback) {
    Community.Behavior.CommentBind($comment_list, publish_id, to_user_id, page, publish_index, callback);
    $.SimpleAjaxPost("service/CommunityService.asmx/GetCommentInfoCount",
        true,
        "{publishID:'" + publish_id + "'}",
        function (json) {
...
Community.Behavior.CommentBind = function get_comment_bind($comment_list, publish_id, to_user_id, page, publish_index, callback) {
        ...
        function (json) {
            ...
            if (Array.isArray(result) == true) {
                ...
                $.each(result, function (index, item) {
                    ...
                    if (item.FromCommenterID == window.objPub.UserID) {
                        $("#sDelComment" + publish_index + "-" + index).on("click", { ID: item.ID, PublishID: publish_id, List: $comment_list, ToUserID: to_user_id, PublishIndex: publish_index, Callback: callback }, Community.Behavior.DeleteCommentEvent);
                    } else {
