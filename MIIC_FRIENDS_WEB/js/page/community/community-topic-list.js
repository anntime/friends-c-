...
Community.Topic.Search = function search(keyword, page) {
    Community.Topic.SearchBind(keyword, page);
        ...
        "{searchView:" + $.Serialize(keyword) + "}",
         function (json) {
             ...
             if (Community.Topic.TotalCount <= Community.Topic.PageSize) {
                 ...
                 if (Community.Topic.CurrentIndex == 0) {
...
Community.Topic.ScrollEvent = function ScorllEvent(event) {
    ...
    if (($(document).scrollTop() >= $(document).height() - $(window).height()) && Community.Topic.OldDocumentHeight != $(document).height()) {
        ...
        else if (parseInt(Community.Topic.TotalCount / Community.Topic.PageSize) > objPub.MinTipPage) {
            $.Alert("这已经是最后一页了哦~");
            $(document).off("scroll");
            setTimeout(function () {
...
Community.Topic.SearchBind = function search_bind(keyword, page) {
    ...
    function (json) {
        ...
        if (result != null) {
            $.each(result, function (index, item) {
                var Index = Community.Topic.CurrentIndex * Community.Topic.PageSize + index;
            ...
            if (Community.Topic.CurrentIndex == 0) {
                $("#sctCommunityTopicList").empty().append(temp);
            } else {
