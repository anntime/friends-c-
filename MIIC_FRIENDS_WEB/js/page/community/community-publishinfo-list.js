...
Community.PublishInfo.PageSize = 10;
//是否能页面加载
    ...
    $.SimpleAjaxPost("service/CommunityService.asmx/GetYears",
       ...
       function (json) {
           ...
           if (Array.isArray(result) == true && result.length > 0) {
               ...
               $(document).on("click", "#aNewestButton", Community.PublishInfo.GetNewestInfosEvent);
               //循环加载年份
               ...
               $(document).on("click", "#aOldestInfos", Community.PublishInfo.GetOldestInfosEvent);
               //goTop 按钮
               temp += "<li id='liGoTop'>";
...
Community.PublishInfo.GetNewestInfosEvent = function GetNewestInfosEvent(event) {
    Community.PublishInfo.GC();
    var page = {
        pageStart: 1,
        pageEnd: Community.PublishInfo.PageSize * 1
...
Community.PublishInfo.GetOldestInfosEvent = function GetOldestInfosEvent(event) {
    Community.PublishInfo.GC();
    var top_view = {
        Top: 1,
        CommunityID: Community.PublishInfo.CommunityID,
...
Community.PublishInfo.HandleResult = function handle_result(result,current_index) {
    ...
    if (result && $.isArray(result) == true && result.length != 0) {
        ...
        $.each(result, function (index, item) {
            ...
            //评论列表
            temp += "<div class='comment-block' id='divCommunityCommentBlock" + Index + "'>";
...
Community.PublishInfo.ShowDetailLongEvent = function ShowDetailLongEvent(event) {
    ...
    objPub.CommunityBrowse(long_obj.ID);
}
