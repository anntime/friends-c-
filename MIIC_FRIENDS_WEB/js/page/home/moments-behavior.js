...
Moments.Behavior.PraiseEvent = function PraiseEvent(event) {
    ...
    var img_$praise_content = event.data.Index ? $("#liMomentsPraise" + event.data.Index).find(".opt-text") : event.data.Index;
        ...
        function (json) {
            ...
            if (result.result == true) {
                if (result.PrimaryID == "") {//取消赞
                    ...
                    if (img_$praise_content) {
                        img_$praise_content.html("赞(<span class='behavior-num'>" + new_praise_num + "</span>)");
                    } 
                } else {//点赞
                    var new_praise_num = praise_num + 1;
                    $praise_content.html("取消赞(<span class='behavior-num'>" + new_praise_num + "</span>)");
                    if (img_$praise_content) {
                        img_$praise_content.html("取消赞(<span class='behavior-num'>" + new_praise_num + "</span>)");
                    }
                }
            }
            else {
...
Moments.Behavior.TreadEvent = function TreadEvent(event) {
    ...
    var img_$thread_content = event.data.Index ? $("#liMomentsTread" + event.data.Index).find(".opt-text") : event.data.Index;
        ...
        function (json) {
            ...
            if (result.result == true) {
                if (result.PrimaryID == "") {
                    ...
                    if (img_$thread_content) {
                        img_$thread_content.html("踩(<span class='behavior-num'>" + new_thread_num + "</span>)");
                    }
                } else {
                    var new_thread_num = thread_num + 1;
                    $thread_content.html("取消踩(<span class='behavior-num'>" + new_thread_num + "</span>)");
                    if (img_$thread_content) {
                        img_$thread_content.html("取消踩(<span class='behavior-num'>" + new_thread_num + "</span>)");
                    }
                }
            }
            else {
...
Moments.Behavior.CollectEvent = function CollectEvent(event) {
    ...
    var img_$collect_content = event.data.Index ? $("#liMomentsCollect" + event.data.Index).find(".opt-text") : event.data.Index;
        ...
        function (json) {
            ...
            if (result.result == true) {
                if (result.PrimaryID == "") {
                    ...
                    if (img_$collect_content) {
                        img_$collect_content.html("收藏(<span class='behavior-num'>" + new_collect_num + "</span>)");
                    }
                } else {
                    var new_collect_num = collect_num + 1;
                    $collect_content.html("取消收藏(<span class='behavior-num'>" + new_collect_num + "</span>)");
                    if (img_$collect_content) {
                        img_$collect_content.html("取消收藏(<span class='behavior-num'>" + new_collect_num + "</span>)");
                    }
                }
            }
            else {
...
Moments.Behavior.ReportEvent = function ReportEvent(event) {
    ...
    var img_$report_content = event.data.Index ? $("#liMomentsReport" + event.data.Index).find(".opt-text") : event.data.Index;
    var publish_id = event.data.PublishID;
       ...
       function (json) {
           ...
           if (result.result == true) {
               if (result.PrimaryID == "") {
                   ...
                   if (img_$report_content) {
                       img_$report_content.html("举报");
                   }
               } else {
                   $report_content.html("取消举报");
                   if (img_$report_content) {
                       img_$report_content.html("取消举报");
                   }
               }
           }
           else {
...
Moments.Behavior.CommentViewEvent = function CommentViewEvent(event) {
    ...
    if ($open.is(":hidden") == false) {
        ...
        if ($(this).data("initial") == undefined) {
            ...
            $("#commentSend" + event.data.PublishIndex).off("click").on("click", { PublishID: publish_id, List: $comment_list, Editor: $comment_div.find(".weibo-editor-comment"), ToUserID: to_user_id, ToUserName: to_user_name, PublishIndex: event.data.PublishIndex }, Moments.Behavior.CommentEvent);
            $("#commentCancel" + event.data.PublishIndex).off("click").on("click", { Editor: $comment_div.find(".weibo-editor-comment"), ToUserID: to_user_id, ToUserName: to_user_name, PublishIndex: event.data.PublishIndex }, Moments.Behavior.CancelCommentEvent).hide();
...
Moments.Behavior.GetComment = function get_comment($comment_list, publish_id, to_user_id, page,publish_index,callback) {
    Moments.Behavior.CommentBind($comment_list, publish_id, to_user_id, page, publish_index, callback);
    //评论查询视图
    var comment_search_view = {
        PublishID : publish_id,
        WithAddress: Enum.YesNo.Yes
...
Moments.Behavior.CommentBind = function get_comment_bind($comment_list, publish_id, to_user_id, page, publish_index, callback) {
    //评论查询视图
    var comment_search_view = {
        PublishID: publish_id,
        WithAddress: Enum.YesNo.Yes
    ...
    $.SimpleAjaxPost("service/MomentsService.asmx/GetCommentInfos",
        ...
        function (json) {
            ...
            if (Array.isArray(result) == true) {
                ...
                $.each(result, function (index, item) {
                    $("#sDelComment" + publish_index + "-" + index).off("click");
                    $("#sReply" + publish_index + "-" + index).off("click");
                    if (item.FromCommenterID == window.objPub.UserID) {
                        $("#sDelComment" + publish_index + "-" + index).on("click", { ID: item.ID, PublishID: publish_id, List: $comment_list, ToUserID: to_user_id, PublishIndex: publish_index,Callback:callback }, Moments.Behavior.DeleteCommentEvent);
                    } else {
