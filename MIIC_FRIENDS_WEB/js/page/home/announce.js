...
Announce.Init = function init() {
    ...
    $(window).off("scroll").on("scroll", { WithTimeAxis: false}, objPub.ScorllEvent);
    var page = {
        pageStart: 1,
...
Announce.ScrollEvent = function ScrollEvent(event) {
    if (($(document).scrollTop() >= $(document).height() - $(window).height()) && Announce.OldDocumentHeight != $(document).height()) {
