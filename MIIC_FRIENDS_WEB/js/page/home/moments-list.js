...
Moments.List.Search = function search(date_view,page) {
    ...
    $.SimpleAjaxPost("service/MomentsService.asmx/GetSearchMyMomentsCount",
        ...
        function (json) {
            var result = json.d;
            Moments.List.TotalCount = result;
            if (result > Moments.List.PageSize) {
...
Moments.List.ScrollEvent = function ScrollEvent(event) {
    ...
    if (($(document).scrollTop() >= $(document).height() - $(window).height()) && Moments.List.OldDocumentHeight != $(document).height()) {
...
Moments.List.SearchBind = function search_bind(date_view, page,current_index) {
    ...
    $.SimpleAjaxPost("service/MomentsService.asmx/SearchMyMoments", true,"{dateView:" + $.Serialize(date_view) + ",page:" + $.Serialize(page) + "}")
        .done(function (json) {
...
Moments.List.HandleResult = function handle_result(json,current_index) {
    ...
    if (result && $.isArray(result) == true && result.length != 0) {
        ...
        $.each(result, function (index, item) {
            ...
            temp += Moments.List.GetAccStr(item.AccInfos,item.ID);
            ...
            //评论列表
            temp += "<div class='comment-block' id='divMomentsCommentBlock"+Index+"'>";
            ...
            temp += "<input value='取消' type='button' class='btn-cancel' id='commentCancel" + Index + "'>&nbsp;&nbsp;";
            temp += "<input value='提交' type='button' class='btn-submit' id='commentSend" + Index + "'>";
            temp += "</div>";
            temp += "</div>";
            temp += "<div class='comment-list'>";
            temp += "<ul>";
            temp += "</ul>";
            temp += "<div class='pagination'><div class='wPaginate8nPosition' class='comment-page'></div></div>";
            temp += "</div>";
            temp += "</div>";
            //评论列表结束
            temp += "</div></li>";
