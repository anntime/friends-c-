...
Moments.Init = function init(is_mine, person) {
    if (is_mine == false) {
        ...
        //切换选项卡
        $(".friend-tabs>a").off("click").on("click", Moments.FriendsTabChangeEvent);
        //访问我发布的信息
        $("#aPublishCount").off("click").on("click", function (event) {
            Moments.List.Person.Init(person ? person.UserID : objPub.UserID);
        });

        //访问通讯录
        $("#aAddressBookCount").off("click").on("click", function (event) {
        ...
        //访问讨论组
        $("#aGroupCount").off("click").on("click", function (event) {
        ...
        //访问行业圈子
        $("#aCommunityCount").off("click").on("click", function (event) {
        ...
        $("#aSetMyTheme").off("click").on("click", Moments.SetMyThemeEvent);
         
          
       
    }
    
    $("#imgUserUrl").off("click").on("click", function (event) {
        Moments.List.Person.Init(person ? person.UserID : objPub.UserID);
    }).css("cursor","pointer");

    //打开通讯录
    $(".contacts").off("click").on("click", function(){
        $(".main-left").load("../biz/left/addressbook/list.html", function (response, status) {
            if (status == "success") {
                AddressBook.Init();
            }
    ...
    //打开发短消息窗口
    $(".message").off("click").on("click", function(event){
        $("#sctShort").dialog("open");
    });
    //打开长篇窗口
    $(".editor").off("click").on("click", function (event) {
        $("#sctLong").dialog("open");
    });
    Moments.List.Init();
    //打开收藏列表
    $("#aCollectList").off("click").on("click", function (event) {
        $(".user-account").hide();
        $("#filter").css({
            "top": "135px"
        });
    ...
    //打开草稿箱
    $("#aDraftList").off("click").on("click", function (event) {
        $(".user-account").hide();
        $("#filter").css({
            "top": "135px"
        });
