...
Moments.List.Person.SetDateList = function set_date_list() {
    ...
    //返回顶部
    $(window).off("scroll").on("scroll", { WithTimeAxis: true ,FromPerson:true}, objPub.ScorllEvent);
    ...
    $.SimpleAjaxPost("service/MomentsService.asmx/GetPersonYears",
       ...
       function (json) {
           ...
           if (Array.isArray(result) == true&&result.length>0) {
               ...
               temp += "</li>";
               $(document).off("click", "#aNewestButton").on("click", "#aNewestButton", Moments.List.Person.GetNewestInfosEvent);
               //循环加载年份
               ...
               temp += "</li>";
               $(document).off("click", "#aOldestInfos").on("click", "#aOldestInfos", Moments.List.Person.GetOldestInfosEvent);
               //goTop 按钮
               temp += "<li id='liGoTop'>";
...
//获取我关注的用户最新博文
Moments.List.Person.GetNewestInfosEvent = function GetNewestInfosEvent(event) {
    //最新取当前年份月份内的所有信息
    Moments.List.Person.GC()
    var date_view = {
        Year: Moments.List.Person.Year,
...
Moments.List.Person.GetOldestInfosEvent = function GetOldestInfosEvent(event) {
    //重置加载
    Moments.List.Person.GC();
    var top_view = {
        Top: 1,
