...
User.Password.ChangePasswordEvent = function ChangePasswordEvent(event) {
    ...
    if ($.IsNullOrEmpty($("#txtNewPsword1").val())) {
        ...
        if (!reg.test($("#txtNewPsword1").val())) {
            $("#txtNewPsword1Tip").html("密码必须为字母、数字、“_”和“.”组成的字符串").show();
            return;
        }
    }

    if ($.IsNullOrEmpty($("#txtNewPsword2").val())) {
        $("#txtNewPsword2Tip").html("请确认新密码").show();
        return;
    } else {
        if (!reg.test($("#txtNewPsword2").val())) {
            $("#txtNewPsword2Tip").html("密码必须为字母、数字、“_”和“.”组成的字符串").show();
            return;
        }
    }

    if ($("#txtNewPsword1").val() != $("#txtNewPsword2").val()) {
    ...
    var passwordView = {
        ...
        Sm3: $.Hex2String(objPub.Sm3Hash.sum($("#txtNewPsword1").val()))
    };
    var isEmail = false;
