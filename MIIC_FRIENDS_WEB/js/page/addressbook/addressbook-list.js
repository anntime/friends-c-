...
AddressBook.Init = function init() {
    ...
    //搜索通讯录事件
    $("#spnShowBlacklistToMyFriendslist").off("click").on("click", { Page: page }, AddressBook.SearchEvent);
    //搜索黑名单事件
    $("#spnMyFriendslistToBlacklist").off("click").on("click", { Page: page }, AddressBook.SearchBlackListEvent);
    //初始化查找用户对话框
...
AddressBook.SetRemark = function set_remark(id,index) {
    var remark_view = {
        ID: id,
...
AddressBook.AddEvent = function AddEvent(event) {
    ...
    //查询通讯录事件
    $("#spnSearchUser").off("click").on("click", { Page: page }, AddressBook.AddFriends.SearchEvent);
