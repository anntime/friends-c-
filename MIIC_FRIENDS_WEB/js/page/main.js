...
//检测登录状态
window.objPub.CheckSsoLogin($.GetCookie("MiicID") ? $.GetCookie("MiicID") : "").done(function (json) {
    var result = json.d;
    if (result != true) {
...
shareHubProxy.on("ReceiveLineNotice", function (user_id) {
    if ($("#sctChat").length > 0) {
...
shareHubProxy.on("ReceiveOfflineNotice", function (user_id) {
    if ($("#sctChat").length > 0) {
        if ($("#liOftenUsedAddressBookChat").hasClass("selected") == true) {
            $.each($("li[id^=liOftenUsedAddressBookChatItem]"), function (index, item) {
                if ($(item).data("UserID").toLowerCase() == user_id.toLowerCase()) {
