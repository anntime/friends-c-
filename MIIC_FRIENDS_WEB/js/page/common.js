//---------------------------设置区域---------------------------//
$.SetBaseUrl("http://share.miic.com.cn/");
//避免缓存文件
$.ajaxSetup({
    cache: false
});
...
///用户类别枚举@end///
//聊天发送格式
Enum.ChatMessageType = function () {
    throw Error.notImplemented();
}
Enum.ChatMessageType.prototype = {
    //文本消息
    Text: 0,
    // 图片消息
    Img: 1,
...
Enum.ChatMessageType.registerEnum("Enum.ChatMessageType");
//聊天上下线状态
Enum.ChatUserStatus = function () {
    throw Error.notImplemented();
}
Enum.ChatUserStatus.prototype = {
    //下线
    Offline: 0,
    //上线
    Line:1
};
Enum.ChatUserStatus.registerEnum("Enum.ChatUserStatus");
///主题级别枚举@start///
...
Enum.BusinessType.GetDescription = function get_description(business_type) {
    var result = "";
    if (isNaN(business_type) == true) {
        throw Error.argumentType("BusinessType", null, Enum.BusinessType, "参数类型不符");
    }
    else {
        switch (business_type) {
            case Enum.BusinessType.Moments:
                result = "朋友圈";
...
window.objPub = {
    UserID: $.GetCookie("MiicID") == undefined ? "" : $.GetCookie("MiicID"),
    ...
    UserName: $.GetCookie("MiicUserName") == undefined ? "" : decodeURI($.GetCookie("MiicUserName")),
    ...
    //朋友圈
    FriendsUrl: "http://share.miic.com.cn/",
    //管理平台
    ManageUrl: "http://sns.miic.com.cn/",
    //微博平台
    MicroblogUrl: "http://weibo.miic.com.cn/",
    OAUrl: "http://oa.miic.com.cn/",
    CurrentUser: null,
    UserLevel: "1",
    IsMain: false,
    Cors: new Array(),
    WindowScrollTop: 0,
    MinTipPage: 1,
    DefaultPhotoUrl: "/file/User/setting.png",
    ServiceID: "1",
    Sm3Hash:new sm3()
};
...
if (objPub.CurrentUser == null
    ...
    if (objPub.UserType == Enum.UserType.PersonUser.toString()
        || objPub.UserType == Enum.UserType.LocalAdmin.toString()
        || objPub.UserType == Enum.UserType.CountryAdmin.toString() 
        || objPub.UserType == Enum.UserType.Administrator.toString()
        ) {
        var url = new Array();
        url.push("http://sns.miic.com.cn/");
...
window.objPub.SetObjPub = function set_objPub() {
    with (objPub) {
        UserID = $.GetCookie("MiicID");
        UserUrl = $.GetCookie("SNS_UserUrl");
        SocialCode = $.GetCookie("SNS_SocialCode");
        UserName = decodeURI($.GetCookie("MiicUserName"));
...
//检测登录状态
window.objPub.CheckSsoLogin = function check_sso_login(user_id) {
    var url = new Array();
    url.push(objPub.ManageUrl);
    url.push("service/UserService.asmx/CheckSsoLogin");
    return $.SimpleAjaxCors(url, "POST", "{userID:'" + user_id + "'}");
}

//sso登录
window.objPub.SsoLoginIn = function sso_login_out(user_id) {
    var url = new Array();
    url.push(objPub.OAUrl);
    url.push("service/UserService.asmx/SsoLoginIn");
    return $.SimpleAjaxCors(url, "POST", "{userID:'" + user_id + "'}");
}

//退出登录
window.objPub.SsoLoginOut = function sso_login_out(user_id) {
    var url = new Array();
    url.push(objPub.OAUrl);
    url.push("service/UserService.asmx/SsoLoginOut");
    return $.SimpleAjaxCors(url, "POST", "{userID:'" + user_id + "'}");
}
...
window.objPub.Logout = function logout() {
    objPub.SsoLoginOut(objPub.UserID).done(function (json) {
        var result = json.d;
        if (result == true) {
            objPub.ClearObjPub();
...
window.objPub.ScorllEvent = function ScorllEvent(event) {
    ...
    if (Right.ScrollTopNum != 0) {
        if ($(this).scrollTop() >= Right.ScrollTopNum) {
            $("#sctHotCommunity").css({
                "top": "0",
                "position": "fixed"
            });
...
//获取民族信息
window.objPub.GetNationList = function get_nation_list() {
    var url = new Array();
    url.push(objPub.ManageUrl);
