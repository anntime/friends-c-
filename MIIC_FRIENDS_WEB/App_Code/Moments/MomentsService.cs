...
public partial class MomentsService : WebService
{
    ...
    [WebMethod(Description = "搜索我的朋友圈信息(含评论)", BufferResponse = true)]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    [GenerateScriptType(typeof(MyDateView))]
    [GenerateScriptType(typeof(MiicPage))]
    public string SearchMyMomentsWithComment(MyDateView dateView, MiicPage page)
    {
        string result = CommonService.InitialJsonList;
        DataTable dt = IpublishInfo.GetPersonMomentsPublishInfosWithComment(dateView, page);
        if (dt.Rows.Count > 0)
        {
            var temp = from dr in dt.AsEnumerable()
                       group dr by
                       ...
                       select new
                       {
                           ...
                           CommentList = (from item in g
                                          select new
                                          {
                                              ID = item["MomentsComment" + Config.Attribute.GetSqlColumnNameByPropertyName<CommentInfo, string>(o => o.ID)],
                                              Content = item[Config.Attribute.GetSqlColumnNameByPropertyName<CommentInfo, string>(o => o.Content)],
                                              FromCommenterID = item[Config.Attribute.GetSqlColumnNameByPropertyName<CommentInfo, string>(o => o.FromCommenterID)],
                                              FromCommenterName = item[Config.Attribute.GetSqlColumnNameByPropertyName<CommentInfo, string>(o => o.FromCommenterName)],
                                              FromCommenterUrl = CommonService.GetManageFullUrl((Convert.IsDBNull(item["FROM_USER_URL"]) ? string.Empty : item["FROM_USER_URL"].ToString())),
                                              FromCommenterType = item["FROM_USER_TYPE"],
                                              FromCommenterRemark = item["FROM_USER_REMARK"],
                                              FromCommenterRealName = item["FROM_USER_REAL_NAME"],
                                              FromCommenterIsFriend = item["FROM_USER_IS_FRIEND"],
                                              CommentTime = Convert.IsDBNull(item[Config.Attribute.GetSqlColumnNameByPropertyName<CommentInfo, DateTime?>(o => o.CommentTime)]) ? null : (DateTime?)item[Config.Attribute.GetSqlColumnNameByPropertyName<CommentInfo, DateTime?>(o => o.CommentTime)],
                                              ToCommenterID = Convert.IsDBNull(item[Config.Attribute.GetSqlColumnNameByPropertyName<CommentInfo, string>(o => o.ToCommenterID)]) ? string.Empty : item[Config.Attribute.GetSqlColumnNameByPropertyName<CommentInfo, string>(o => o.ToCommenterID)].ToString(),
                                              ToCommenterName = Convert.IsDBNull(item[Config.Attribute.GetSqlColumnNameByPropertyName<CommentInfo, string>(o => o.ToCommenterName)]) ? string.Empty : item[Config.Attribute.GetSqlColumnNameByPropertyName<CommentInfo, string>(o => o.ToCommenterName)].ToString(),
                                              ToCommenterUrl = CommonService.GetManageFullUrl((Convert.IsDBNull(item["TO_USER_URL"]) ? string.Empty : item["TO_USER_URL"].ToString())),
                                              ToCommenterType = item["TO_USER_TYPE"],
                                              ToCommenterRemark = item["TO_USER_REMARK"],
                                              ToCommenterRealName = item["TO_USER_REAL_NAME"],
                                              ToCommenterIsFriend = item["TO_USER_IS_FRIEND"]
                                          })
                       };
    ...
    [WebMethod(Description = "搜索我的朋友圈信息数（含评论）", BufferResponse = true)]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    [GenerateScriptType(typeof(MyDateView))]
    public int GetSearchMyMomentsWithCommentCount(MyDateView dateView)
    {
        return IpublishInfo.GetPersonMomentsPublishWithCommentCount(dateView);
    }

    [WebMethod(Description = "搜索好友的朋友圈信息(含评论)", BufferResponse = true)]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    [GenerateScriptType(typeof(PersonDateView))]
    [GenerateScriptType(typeof(MiicPage))]
    public string SearchPersonMomentsWithComment(PersonDateView dateView, MiicPage page)
    {
        string result = CommonService.InitialJsonList;
        DataTable dt = IpublishInfo.GetPersonMomentsPublishInfosWithComment(dateView, page);
        if (dt.Rows.Count > 0)
    ...
    [WebMethod(Description = "搜索好友的朋友圈信息数（含评论）", BufferResponse = true)]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    [GenerateScriptType(typeof(PersonDateView))]
    public int GetSearchPersonMomentsWithCommentCount(PersonDateView dateView)
    {
        return IpublishInfo.GetPersonMomentsPublishWithCommentCount(dateView);
    }
