...
public class AddressBookService : WebService
{
    ...
    private static readonly IOpenIMAddressbookInfo IopenIMAddressbookInfo = new OpenAddressbookInfoService();
    ...
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public string ShowMyOftenUsedAddressBookList(int top)
    {
        string result = CommonService.InitialJsonList;
        DataTable dt = IaddressBookInfo.GetOftenUsedAddressBookList(this.UserID, top);
        if (dt.Rows.Count > 0)
        {
            var temp = from dr in dt.AsEnumerable()
                       select new
    ...
    [WebMethod(Description = "获取我的经常联系的通讯录列表", BufferResponse = true)]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public string GetMyAllOftenUsedAddressBookList()
    {
        string result = CommonService.InitialJsonList;
        DataTable dt = IaddressBookInfo.GetAllOftenUsedAddressBookList(this.UserID);
        if (dt.Rows.Count > 0)
        {
            var temp = from dr in dt.AsEnumerable()
                       select new
    ...
    [WebMethod(Description = "获取通讯录之间的聊天记录", BufferResponse = true)]
    [GenerateScriptType(typeof(OpenIMChatLogsRequestInfo))]
    [GenerateScriptType(typeof(RoamingMessageResult))]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]

    public RoamingMessageResult GetChatLogs(OpenIMChatLogsRequestInfo chatLogsRequestInfo)
    {
        
        chatLogsRequestInfo = Config.Attribute.ConvertObjectWithDateTime<OpenIMChatLogsRequestInfo>(chatLogsRequestInfo);
        chatLogsRequestInfo.FromUser = chatLogsRequestInfo.FromUser.ToLower();
        chatLogsRequestInfo.ToUser = chatLogsRequestInfo.ToUser.ToLower();
        chatLogsRequestInfo.EndTime = chatLogsRequestInfo.EndTime.Value.AddHours(1);
        //RoamingMessageResult result = IopenIMAddressbookInfo.GetChatLogs(chatLogsRequestInfo);
        //result.Messages = result.Messages.OrderBy(o => o.Time).ToList<RoamingMessage>();
        return new RoamingMessageResult() { };
    }
    [WebMethod(Description = "获取我的所有通讯录好友", BufferResponse = true)]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public string GetMyAllAddressBookList()
    {
        ...
        string result = CommonService.InitialJsonList;
        DataTable dt = IaddressBookInfo.GetPersonAllAddressBookList(this.UserID);
        if (dt.Rows.Count > 0)
        {
            var temp = from dr in dt.AsEnumerable()
                       select new
