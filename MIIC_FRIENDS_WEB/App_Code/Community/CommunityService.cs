...
public partial class CommunityService : WebService
{
    ...
    public CommunityService()
    {
        ...
        {
            this.UserName = cookie.GetCookie("MiicUserName", out message);
        }
    }
    ...
    [WebMethod(Description = "获取中心OA展示话题", BufferResponse = true)]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public string GetMiicCommunityLabels()
    {
        string result = CommonService.InitialJsonList;

        List<MiicKeyValue> labelList = new List<MiicKeyValue>();
        //机械工业信息中心 圈子ID
        string communityID = "38714014-a1fd-c762-7646-6d6eae06c523";
        DataTable dt = IlabelInfo.GetLabelInfosByCommunityID(communityID);
        for (int i = 0, len = dt.Rows.Count; i < len; i++)
        {
            //话题1
            labelList.Add(new MiicKeyValue()
            {
                Name = dt.Rows[i][Config.Attribute.GetSqlColumnNameByPropertyName<LabelInfo, string>(o => o.ID)].ToString(),
                Value = dt.Rows[i][Config.Attribute.GetSqlColumnNameByPropertyName<LabelInfo, string>(o => o.LabelName)].ToString()
            });
        }
        ...
        result = Config.Serializer.Serialize(new
        {
            ...
            CommunityName = "机械工业信息中心",
            LabelList = labelList
        });
        return result;
    }

    [WebMethod(Description = "搜索中心OA最新的行业圈信息（长篇）", BufferResponse = true)]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    [GenerateScriptType(typeof(CommunityTopView))]
    public string GetMiicNewestCommunityPublishInfo(CommunityTopView topView)
    {
        string result = CommonService.InitialJsonList;
        DataTable dt = IpublishInfo.GetMiicNewestCommunityPubilishInfos(topView);
        if (dt.Rows.Count > 0)
        {
            var temp = from dr in dt.AsEnumerable()
                       group dr by
                       new
                       {
                           ...
                           Content = dr[Config.Attribute.GetSqlColumnNameByPropertyName<PublishInfoWithUserInfo, string>(o => o.Content)].ToString(),
                       ...
                       select new
                       {
                           ...
                           AccInfos = (from item in g
                                       ...
                                       select new
                                       {
                                           CommunityPublishAccessoryInfoID = item["CommunityPublishAccessoryInfo" + Config.Attribute.GetSqlColumnNameByPropertyName<AccessoryInfo, string>(o => o.ID)].ToString(),
