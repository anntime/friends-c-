    ...
    public partial class GroupInfoDao : RelationCommon<GroupInfo, GroupMember>, IGroupInfo
    {
        ...
        bool ICommon<GroupInfo>.Insert(GroupInfo groupInfo)
        {
            ...
            if (result == true)
            {
                ...
                members.Add(groupInfo.CreaterID);
                string tribeID = string.Empty;
                //IopenIMGroupInfo.Create(new OpenIMGroupInfo()
                //{
                //    GroupName = groupInfo.Name,
                //    UserID = groupInfo.CreaterID,
                //    Members = members,
                //    Notice = "欢迎加入" + groupInfo.Name + "讨论组~"
                //}, out tribeID);
                UpdateTribeID(groupInfo.ID, tribeID);
                List<GroupMember> cache = new List<GroupMember>();
                cache.Add(member);
                InsertCaches(cache);
                groupInfo.TribeID = tribeID;
                InsertCache(groupInfo, cache);

            }
            return result;
        }
        bool UpdateTribeID(string groupID, string tribeID)
        {
            bool result = false;
            int count = 0;
            string message = string.Empty;
            try
            {
                result = dbService.Update(new GroupInfo()
                {
                    ID = groupID,
                    TribeID = tribeID
        ...
        bool ICommon<GroupInfo>.Update(GroupInfo groupInfo)
        {
            ...
            int count = 0;
            string message = string.Empty;
            GroupInfo old = ((ICommon<GroupInfo>)this).GetInformation(groupInfo.ID);
            try
            {
            ...
            if (result == true)
            {
                ...
                if (old.Name != groupInfo.Name && !string.IsNullOrEmpty(groupInfo.Name))
                {
                    string groupCode = string.Empty;
                    //IopenIMGroupInfo.Update(groupInfo.TribeID, new OpenIMGroupInfo()
                    //{
                    //    GroupName = groupInfo.Name,
                    //    UserID = groupInfo.CreaterID,
                    //    Notice = groupInfo.CreaterName + "修改了讨论组名称~"
                    //}, out groupCode);
                }
            }
            return result;
        ...
        public bool Insert(GroupInfo groupInfo, List<GroupMember> members)
        {
            ...
            if (result == true)
            {
                List<string> group_members = new List<string>();
                group_members.Add(groupInfo.CreaterID);
                foreach (var item in members)
                {
                    group_members.Add(item.MemberID);
                }
                string tribeID = string.Empty;
        ...
        DataTable IGroupInfo.GetGroupInfoList(string userID, int top)
        {
        ...
        DataTable IGroupInfo.GetPersonAllGroupList(string userID)
        {
            Contract.Requires<ArgumentNullException>(!string.IsNullOrEmpty(userID), "参数userID:不能为空");
            DataTable result = new DataTable();
            string message = string.Empty;
            ...
            MiicColumn logoUrlColumn = new MiicColumn(Config.Attribute.GetSqlTableNameByClassName<GroupInfo>(),
                Config.Attribute.GetSqlColumnNameByPropertyName<GroupInfo, string>(o => o.LogoUrl));
            columns.Add(logoUrlColumn);
            MiicColumn tribeIDColumn = new MiicColumn(Config.Attribute.GetSqlTableNameByClassName<GroupInfo>(),
               Config.Attribute.GetSqlColumnNameByPropertyName<GroupInfo, string>(o => o.TribeID));
            columns.Add(tribeIDColumn);
            MiicColumn remarkColumn = new MiicColumn(Config.Attribute.GetSqlTableNameByClassName<GroupMember>(),
                Config.Attribute.GetSqlColumnNameByPropertyName<GroupMember, string>(o => o.Remark));
            columns.Add(remarkColumn);
            try
        ...
        int IGroupInfo.GetPersonAllGroupCount(string userID)
        {
            Contract.Requires<ArgumentNullException>(!string.IsNullOrEmpty(userID), "参数userID:不能为空");
            int result = 0;
            string message = string.Empty;
            ...
            try
            {
                result = dbService.GetCount(column, relation, conditions, out message, true);
            }
            catch (Exception ex)
            {
                Config.IlogicLogService.Write(new LogicLog()
        ...
        //导入，返回布尔，out错误list
        bool IGroupInfo.CopyGroupUserInfoToAliService(string groupID)
        {
            bool result = false;
            //获取某群组中所有的用户list
            List<GroupMember> groupMemberList = ((IGroupInfo)this).GetGroupMemberList(groupID);
            //获得群组信息
            GroupInfo groupInfo = ((ICommon<GroupInfo>)this).GetInformation(groupID);
            //群组用户id
            List<string> members = new List<string>();
            foreach (GroupMember memberInfo in groupMemberList)
            {
                members.Add(memberInfo.MemberID);
            }
            //result = IopenIMGroupInfo.Create(new OpenIMGroupInfo()
            //    {
            //        GroupName = groupInfo.Name,
            //        UserID = groupInfo.CreaterID,
            //        Members = members,
