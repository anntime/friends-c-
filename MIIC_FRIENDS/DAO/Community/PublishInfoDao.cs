...
namespace Miic.Friends.Community
{
    public partial class PublishInfoDao : RelationCommon<PublishInfo, AccessoryInfo>, IPublishInfo
    {
                ...
                if (noticeUserView != null && noticeUserView.Noticers != null)
                {
                    ...
                    foreach (var item in noticeUserView.Noticers.AsEnumerable())
                    {
                        sqls.Add(DBService.InsertSql<NoticeInfo>(new NoticeInfo()
                        {
                            ...
                            ReadStatus = ((int)MiicYesNoSetting.No).ToString()
                        }, out message4));
                    }
                }
        ...
        /// 马技超
        /// </summary>
        /// <param name="topView">top视图</param>
        /// <returns>发布信息列表</returns>
        DataTable IPublishInfo.GetMiicNewestCommunityPubilishInfos(CommunityTopView topView)
        {
            Contract.Requires<ArgumentOutOfRangeException>(topView != null, "参数topView:不能为空");
            Contract.Requires<ArgumentOutOfRangeException>(topView.Top > 0, "参数topView.Top:必须为正整数");
            DataTable result = new DataTable();
            ...
            MiicCondition validCondition = new MiicCondition(Config.Attribute.GetSqlColumnNameByPropertyName<PublishLabelRelation, string>(o => o.Valid),
                ((int)MiicValidTypeSetting.Valid).ToString(),
                 DbType.String,
                  MiicDBOperatorSetting.Equal);
            condition.Add(new MiicConditionLeaf(validCondition));
            MiicCondition editStatusCondition = new MiicCondition(Config.Attribute.GetSqlColumnNameByPropertyName<PublishInfoWithUserInfo, string>(o => o.EditStatus),
                                                      ((int)MiicYesNoSetting.No).ToString(),
                                                      DbType.String,
                                                      MiicDBOperatorSetting.Equal);
            condition.Add(new MiicConditionLeaf(editStatusCondition));
            MiicCondition publishTypeCondition = new MiicCondition(Config.Attribute.GetSqlColumnNameByPropertyName<PublishInfoWithUserInfo, string>(o => o.PublishType),
                                                      ((int)PublishInfoTypeSetting.Long).ToString(),
                                                      DbType.String,
                                                      MiicDBOperatorSetting.Equal);
            condition.Add(new MiicConditionLeaf(publishTypeCondition));

            MiicOrderBy publishTimeOrder = new MiicOrderBy()
            {
                 Desc = true,
                 PropertyName = Config.Attribute.GetSqlColumnNameByPropertyName<PublishInfoWithUserInfo, DateTime?>(o => o.PublishTime)
            };

            List<MiicOrderBy> order = new List<MiicOrderBy>();
            order.Add(publishTimeOrder);
            ...
            try
            {
                result = dbService.GetInformationsPage(column, relation, condition, new MiicPage()
                {
                    pageStart = "1",
                    pageEnd = topView.Top.ToString()
                }, out message, MiicDBPageRowNumberSetting.DenseRank);
            }
            catch (Exception ex)
            {
                LogicLog log = new LogicLog()
